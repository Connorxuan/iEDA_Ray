...
def objective(trial):
    ...
    num_cpus = trial.suggest_int("num_cpus", 1, 2)
    memory = trial.suggest_categorical("memory", [128, 256, 512, 1024]) * 1024 * 1024
    strategy = trial.suggest_categorical("scheduling_strategy", ["DEFAULT", "SPREAD"])

    start_time = time.time()
    ray.get(main.options(num_cpus=num_cpus, memory=memory, scheduling_strategy=strategy).remote(env_vars))
    end_time = time.time()
...
if __name__ == "__main__":
    study = optuna.create_study(directions=["minimize", "minimize", "minimize"],study_name="1222",storage="sqlite:///data.db",load_if_exists=True)
    def worker():
    ...
    end_time = time.time()
    time_cost = end_time - start_time
    result_str = "Multi-process parallel (consider resource allocation) runtime: %.2f s" % time_cost
    with open('ChipPilotReport-mix.txt', 'w') as file:
