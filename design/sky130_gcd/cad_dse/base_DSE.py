...
def objective(trial):
    ...
    with open(f'{result_dir}/cts/sta/gcd.rpt', 'r') as file:
        #     ^o^v ^v^g    ^f^e          ^m     ^h^w   
    ...
    with open(f'{result_dir}/report/cts_db.rpt', 'r') as file:
        #     ^o^v ^v^g    ^f^e          ^m     ^h^w   
        lines = file.readlines()
        #   ^` ^=  ^v^g    ^x  ^p  ^g   ^q ^|^i5  ^l
        if len(lines) >= 5:
...
if __name__ == "__main__":
    ...
    result_str = "Single-process serial runtime(1st time): %.2f s" % time_cost
    with open('BaseDSEReport.txt', 'w') as file:
        file.write(result_str + '\n')
    print(result_str)

    time_predict = time_cost * 100
    result_str = "Single-process serial prediction runtime%.2f s" % time_predict
